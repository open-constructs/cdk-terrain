...
Uses `@cdktn/hcl2cdk` to transform HCL configuration to CDK constructs.
...
```ts
import { convert } from "@cdktn/hcl2cdk";
...
new cdktn.TerraformVariable(this, "imageId", {
  type: "string",
...
This transforms your Terraform project into a CDK Terrain project, besides the resource naming within Terraform the deployed resources should not differ between `terraform plan` and `cdktn plan`.
