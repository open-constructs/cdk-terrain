...
import { TerraformLogin } from "./terraform-login";
...
import { templates, templatesDir } from "@cdktn/cli-core";
...
async function askForProviders(): Promise<string[] | undefined> {
  ...
  console.log(
    chalkColour`{yellow Note: You can always add providers using 'cdktn provider add' later on}`,
  );
...
async function gatherInfo(
  ...
  const projectDescriptionDefault =
    "A simple getting started project for cdktn.";
