...
const nonIteractiveLoginWithNoTokenError = (invalid: boolean) =>
  Errors.Usage(
    ...
    }  Terraform Cloud. This can also happen when running cdktn convert against a project not using Typescript, since we need to create a temporary cdktn project for an accurate translation. Please run 'cdktn login' to log in.`,
  );
...
export class TerraformLogin {
  ...
  public async askToContinue(): Promise<boolean> {
    ...
    console.log(chalkColour`{greenBright Welcome to CDK Terrain!}
...
If login is successful, cdktn will store the token in plain text in
