...
export async function verifySimilarLibraryVersion() {
  ...
  let libVersion = await getPackageVersion(language, "cdktn");
  if (!libVersion) {
    // fall back to cdktf version
    libVersion = await getPackageVersion(language, "cdktf");
  }
  if (!libVersion) {
  ...
  if (cliVersion === "0.0.0" || cliVersion.includes("-dev")) {
    logger.debug(
      `Running a development version of cdktn, skipping version check`,
    );
    return;
  }
  ...
  if (cliVersion.includes(".dev") || cliVersion.includes("-pre.")) {
    logger.debug(
      `Running a pre-release version of cdktn-cli, skipping version compatibility check`,
    );
    return;
  }
  ...
  if (libVersion.includes(".dev") || libVersion.includes("-pre.")) {
    logger.debug(
      `Running a pre-release version of cdktn, skipping version compatibility check`,
    );
    return;
  }
