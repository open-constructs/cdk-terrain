...
import { readConfigSync } from "@cdktn/commons";
  ...
  public readonly builder = (args: yargs.Argv) =>
    args
      ...
      .option("app", {
        default: config.app,
        required: true,
        desc: "Command to use in order to execute cdktn app",
        alias: "a",
      })
      .option("output", {
      ...
      .option("parallelism", {
        ...
        type: "number",
        required: false,
        desc: "Number of concurrent CDKTN stacks to run. Defaults to infinity, denoted by -1",
        default: -1,
      })
      .option("refresh-only", {
        default: false,
      ...
      // causes colors to be disabled too for output that the cdktn-cli itself colors.
      .option("no-color", {
        type: "boolean",
