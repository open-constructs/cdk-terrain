...
/**
 * Drive a long-running `cdktn watch` session. Sits in an event loop that
 * triggers auto-approved deploys on file changes, surfacing each deploy's
 * progress in the pinned status bar.
 *
 * The session ends when the user sends SIGINT/SIGTERM/SIGQUIT, which is
 * propagated to cli-core via an AbortController.
 *
 * @param config - Watch options. `autoApprove` is required (the wrapper in
 *   `handlers.ts` rejects invocations that omit it).
 * @returns Promise that resolves on graceful shutdown, rejects on
