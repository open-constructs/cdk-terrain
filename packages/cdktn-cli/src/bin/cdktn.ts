...
yargs
  ...
  .option("experimental-provider-schema-cache-path", {
    ...
    default: false,
    required: false,
    desc: "An experimental schema cache that can be used to improve the speed of cdktn get and convert. Supported using the env CDKTF_EXPERIMENTAL_PROVIDER_SCHEMA_CACHE_PATH.",
  })
  ...
  .option("log-file-directory", {
    require: false,
    desc: "The directory path where CDKTN should create `cdktf.log` and print logs at the `debug` level. If not set, CDKTF writes logs to standard out at the level specified in `CDKTF_LOG_LEVEL`. Only supported via setting the env CDKTF_LOG_FILE_DIRECTORY",
  })
  .option("context-json", {
    required: false,
    hidden: true,
  ...
  .completion("completion", customCompletion) // outputs completion script on "cdktn completion"
  .parserConfiguration({
    ...
    handler: (argv) => {
      ...
      console.error(
        cmd
          ...
          : `Please pass a command to cdktn, here are all available ones:`,
      );
      yargs.showHelp();
      process.exit(1);
    },
